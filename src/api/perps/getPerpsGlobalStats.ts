...
export default async function getPerpsGlobalStats(timeframe: string = '30') {
  ...
  try {
    ...
    const timeframeConfig = TIMEFRAME.find((t) => t.value === timeframe) || {
      granularity: 'day',
      unit: Number(timeframe),
    }

    const url = new URL(
      `${baseUrl}/v2/perps_overview?chain=neutron&granularity=${timeframeConfig.granularity}&unit=${timeframeConfig.unit}&product=creditmanager&response_type=global`,
    )
